 ...
 * - Authenticates to Google Drive via OAuth Refresh Token or Service Account JWT
...
const VIDEO_PATH = path.resolve(getParam("video-path", "VIDEO_PATH", "out.mp4"));
const PROMPT = getParam("prompt", "PROMPT", "Generated Video");
const VIDEO_ID = getParam("video-id", "VIDEO_ID", "video-" + Date.now());
const TARGET_FOLDER_ID = getParam(
  "gdrive-main-folder-id",
  "GDRIVE_MAIN_FOLDER_ID",
  "1JGjibA287ds3SFoT_Fl2z8cJ96eCDUFs"
);
const SUPABASE_URL = (getParam("supabase-url", "SUPABASE_URL")).replace(/\/+$/, "");
const SUPABASE_SERVICE_ROLE_KEY = getParam(
  "supabase-service-role-key",
  "SUPABASE_SERVICE_ROLE_KEY"
);
const GOOGLE_CLIENT_ID = getParam("google-client-id", "GOOGLE_CLIENT_ID");
const GOOGLE_CLIENT_SECRET = getParam("google-client-secret", "GOOGLE_CLIENT_SECRET");
const GOOGLE_REFRESH_TOKEN = getParam("google-refresh-token", "GOOGLE_REFRESH_TOKEN");
const GDRIVE_CLIENT_EMAIL = getParam("gdrive-client-email", "GDRIVE_CLIENT_EMAIL");
const GDRIVE_PRIVATE_KEY = getParam("gdrive-private-key", "GDRIVE_PRIVATE_KEY").replace(/\\n/g, "\n");
...
async function patchSupabase(fields = {}) {
  ...
  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY || !isUuid) {
    console.log(`[Supabase] Skipping update (Valid UUID: ${isUuid}, URL: ${!!SUPABASE_URL})`);
    return;
  }
  try {
...
async function findOrCreateVideosFolder(accessToken, parentFolderId) {
  try {
    const q = encodeURIComponent(
      `mimeType='application/vnd.google-apps.folder' and name='Videos' and '${parentFolderId}' in parents and trashed=false`
    );
...
async function uploadVideoToDrive() {
  ...
  console.log(
    `[Drive] Starting upload for ${VIDEO_PATH} (${(fileSize / (1024 * 1024)).toFixed(2)} MB)...`
  );
  ...
  const cleanTitle =
    PROMPT.replace(/[^a-zA-Z0-9\-\s]/g, "")
      .trim()
      .substring(0, 50) || "Hyper-Video";
