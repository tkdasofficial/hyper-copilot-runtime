...
function renderWithFfmpegMaster(manifest) {
  ...
  const audioFile = manifest.audioFile && fs.existsSync(manifest.audioFile)
    ? manifest.audioFile
    : (manifest.voiceFile || '');
  const assFile = manifest.captionsAss && fs.existsSync(manifest.captionsAss)
    ? manifest.captionsAss
    : '';
  ...
  console.log(`[Render] Pre-processing ${scenes.length} scene clips with Ken Burns pan/zoom and 16:9 canvas...`);
  for (let i = 0; i < scenes.length; i++) {
    ...
    if (zoomType === 'zoom_in') {
      ...
      // Pan Left to Right
      const wPad = Math.round(width * 1.10);
      const hPad = Math.round(height * 1.10);
  ...
  console.log(`[Render] Final master compilation completed in ${((Date.now() - t0) / 1000).toFixed(1)}s`);
}
