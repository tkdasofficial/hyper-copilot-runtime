...
Respond ONLY with valid JSON in the format: {"scenes": [{"query": "...", "narration": "..."}]}`;
      ...
      const res = await new Promise((resolve, reject) => {
        const req = https.request(new URL(cfUrl), {
          method: 'POST',
          headers: {
      ...
      if (match) {
        ...
        if (Array.isArray(parsed.scenes) && parsed.scenes.length >= numScenes - 2) {
          console.log(`[Storyboard] Successfully acquired ${parsed.scenes.length} scenes from Cloudflare AI`);
...
function buildDomainStoryboard(promptText, count) {
  ...
  if (p.includes('milky way') || p.includes('galaxy') || p.includes('space') || p.includes('universe') || p.includes('cosmos') || p.includes('star')) {
    const spacePool = [
      { q: "deep space dark matter cosmic web universe", n: "Over thirteen billion years ago, in the violent dawn of the cosmos, primordial gas drifted across halos of invisible dark matter." },
      { q: "nebula glowing star birth space hubble", n: "Vast pockets of hydrogen and helium collapsed under immense gravity, igniting the very first generation of stars." },
      { q: "protogalaxy cosmic collision space dust", n: "Protogalactic fragments collided and coalesced, sparking intense gravitational shockwaves across the early universe." },
      { q: "spiral galaxy spin stars astronomy cosmos", n: "Angular momentum caused the colossal maelstrom of gas and stars to flatten into an expansive rotating cosmic disc." },
      { q: "supermassive black hole accretion disk gravity", n: "At the nucleus, an immense gravitational titan formed: Sagittarius A*, anchoring the evolving galaxy." },
      { q: "stellar nursery glowing nebula cosmic dust", n: "Dense molecular clouds formed along spiral density waves, continuously birthing generations of new stellar systems." },
      { q: "supernova cosmic explosion star death nebula", n: "Massive stars reached the end of their lifespans, detonating as supernovae and seeding space with heavy elements." },
      { q: "interstellar medium dust clouds galaxy 4k", n: "These enriched stellar remnants formed complex planetary nebulae, preparing the cosmos for rocky planets." },
      { q: "solar system sun planets forming space", n: "Five billion years ago, in the Orion spiral arm, our Solar System condensed from a spinning protoplanetary disk." },
      { q: "milky way galaxy spiral arms cosmos 4k", n: "Today, the Milky Way thrives as a majestic stellar metropolis, harboring over one hundred billion stars." },
      { q: "earth night sky milky way stars timelapse", n: "From our vantage point on Earth, this breathtaking galactic river arcs across our clear night skies." },
      { q: "andromeda galaxy collision space future", n: "In four billion years, our galaxy will merge with Andromeda in an epic dance, forming a colossal elliptical supergalaxy." },
      { q: "infinite universe galaxies deep field hubble", n: "The Milky Way remains an enduring testament to the wondrous, creative force of cosmic evolution." }
    ];
    return interpolateScenes(spacePool, count);
  }

  if (p.includes('sea') || p.includes('ocean') || p.includes('deep sea') || p.includes('marine') || p.includes('underwater')) {
    const oceanPool = [
      { q: "deep ocean underwater sunbeams blue water", n: "Beneath the restless surface of the open ocean lies Earth's most enigmatic and untouched frontier." },
      { q: "coral reef tropical fish marine ecosystem", n: "Sunlit coral reefs burst with dazzling biodiversity, functioning as vibrant underwater cities of life." },
      { q: "twilight zone deep ocean dark blue waters", n: "Descending deeper into the twilight zone, sunlight vanishes, surrendering to eternal darkness and pressure." },
      { q: "bioluminescent jellyfish glowing deep sea creature", n: "Bioluminescent organisms illuminate the black depths with spectral living light to hunt and communicate." },
      { q: "hydrothermal vent deep ocean volcanic smoke", n: "Along the ocean floor, hydrothermal vents spew superheated minerals, sustaining otherworldly chemosynthetic communities." },
      { q: "giant squid deep sea abyss creature underwater", n: "In the immense abyssal plains, stealthy predators navigate the quiet expanse of extreme cold and pressure." },
      { q: "whale migrating deep ocean marine life", n: "Great marine mammals traverse thousands of miles across oceanic highways, connecting global marine ecosystems." },
      { q: "underwater kelp forest sea otters sunlight", n: "Towering kelp forests sway in rhythm with coastal currents, sheltering countless marine species." },
      { q: "ocean waves sunset aerial drone cinematic", n: "The deep sea remains the vital life support system of our planet, driving our climate and supporting life on Earth." }
    ];
  ...
  const genericPool = [
    { q: `${promptText} cinematic documentary 4k`, n: `From the beginning of recorded understanding, ${promptText} has captivated our collective imagination.` },
    { q: `${promptText} details perspective macro cinematic`, n: `Beneath the surface lies a rich complexity that shapes our modern perception and history.` },
    { q: `${promptText} motion atmosphere majestic`, n: `Every subtle shift and dynamic transformation reveals another layer of this extraordinary subject.` },
    { q: `${promptText} light shadow contrast cinematic`, n: `Through innovative exploration and focused observation, new revelations continue to unfold.` },
    { q: `${promptText} wide view landscape horizon`, n: `As we look to the horizon, the enduring significance of this subject inspires our future vision.` }
  ];
...
async function synthesizeNarrationAndSubtitles(scenes) {
  ...
  try {
    ...
    try {
      execSync(`edge-tts --voice "${voice}" -f "${scriptPath}" --write-media "${mp3Path}" --write-subtitles "${vttPath}"`, { stdio: 'inherit' });
    } catch (e2) {
      console.warn(`[TTS] Secondary edge-tts failed. Generating procedural voice placeholder.`);
      execSync(`ffmpeg -y -f lavfi -i "sine=frequency=300:duration=${Math.max(10, scenes.length * 5)}" -c:a libmp3lame -b:a 192k "${mp3Path}"`, { stdio: 'ignore' });
    }
  }
...
async function searchStockClip(query, index) {
        ...
        for (const vid of res.videos) {
          if (!usedClipIds.has(String(vid.id))) {
            ...
            if (best && best.link) {
              ...
              console.log(`[Pexels] Found clip for "${query}" (ID: ${vid.id}, ${best.width}x${best.height})`);
      ...
      if (res && res.hits && res.hits.length > 0) {
        for (const hit of res.hits) {
          if (!usedClipIds.has(String(hit.id))) {
            ...
            const chosen = vids.large?.url ? vids.large : (vids.medium?.url ? vids.medium : vids.small);
  ...
  const colors = ['#0a1128', '#001f54', '#034078', '#1282a2', '#00293c', '#1b1b2f', '#1f4068', '#162447'];
  const col = colors[index % colors.length];
  execSync(`ffmpeg -y -f lavfi -i "color=c=${col}:s=1920x1080:d=6,drawbox=x=0:y=0:w=1920:h=1080:color=white@0.03:t=fill" -c:v libx264 -preset ultrafast -pix_fmt yuv420p "${destPath}"`, { stdio: 'ignore' });
  return destPath;
}
...
async function mixAudioWithDucking(narrationMp3Path, targetDuration) {
  ...
  console.log('[Audio] Mixing Voiceover and Cinematic BGM with dynamic Audio Ducking (sidechaincompress)...');
  ...
  try {
    ...
    execSync(`ffmpeg -y -i "${narrationMp3Path}" -c:a aac -b:a 192k "${mixedAudioPath}"`, { stdio: 'ignore' });
  }
...
async function main() {
  ...
  const { mp3Path, words, assPath, srtPath, jsonPath } = await synthesizeNarrationAndSubtitles(scenes);

  // Determine actual audio duration
  let voiceDuration = DURATION_SECONDS;
  try {
    const probe = execSync(`ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${mp3Path}"`).toString().trim();
  ...
  fs.writeFileSync(path.join(ASSETS_DIR, 'timeline.json'), JSON.stringify(timelineData, null, 2), 'utf-8');
